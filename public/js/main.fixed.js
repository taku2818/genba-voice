...
function init() {
    ...
    // submitButtonがある場合は無効化しておく（データ入力後に有効化）
    if (submitButton) {
...
function createReportWithAudio(audioBlob) {
    // API設定が利用可能か確認
    if (!window.API || !window.API.OPENAI_API_KEY) {
        console.error('API設定が見つかりません');
        alert('API設定が見つかりません。APIキーを設定してください。');
        return;
    }
